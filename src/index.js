...
function onerror(err, req, res) {
  ...
  res.setHeader("Allow", `${req.allowedMethod}`);
  res.end(err.message);
}
...
export default function factory({
  onError = onerror,
  onNoMatch = onerror.bind(null, { status: 404, message: " method not found" }),
  attachParams = false,
} = {}) {
  let allowedMethod = [];
  function nc(req, res) {
    return nc.run(req, res).then(
      () => !isResSent(res) && onNoMatch({ req, ...allowedMethod }, res),
  ...
  nc.copy = add.bind(nc, "COPY");
  nc.link = add.bind(nc, "LINK");
  nc.unlink = add.bind(nc, "UNLINK");
  nc.purge = add.bind(nc, "PURGE");
  nc.lock = add.bind(nc, "LOCK");
  nc.unlock = add.bind(nc, "UNLOCK");
